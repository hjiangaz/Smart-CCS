...
Image.MAX_IMAGE_PIXELS = 9999331200000
...
class WholeSlideImage(object):
    def __init__(self, path, core):
        ...
        Args:
            ...
            core (str): cpu core to use for m
        """
        self.core = core
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        if len(results)>1:
            ...
            pool = mp.Pool(num_workers)
            slide_path = self.wsi.filepath
            patch_save_path = os.path.join(save_path, slide_path.split('/')[-1].split('.')[0])
            if os.path.exists(patch_save_path):
                ...
                os.makedirs(patch_save_path)
            iterable = [(coord, patch_level, patch_save_path, slide_path, patch_size) for coord in results]
            pool.starmap(WholeSlideImage.save_coord_candidate, iterable)
            pool.close()
    ...
    @staticmethod
    def save_coord_candidate(coord, patch_level, save_path, slide_path, patch_size):
        slide = Slide(slide_path)
        patch = slide.read_region(coord, patch_level, (patch_size, patch_size))

        patch = np.array(patch)
        patch = cv2.cvtColor(patch, cv2.COLOR_BGR2RGB)
        cv2.imwrite(os.path.join(save_path, '{}_{}_{}.jpg'.format(coord[0], coord[1], patch_size)), patch)
